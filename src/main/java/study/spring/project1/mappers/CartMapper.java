...
public interface CartMapper {
    ...
    @Insert("INSERT INTO cart (id, user_id, product_id, adate, cnt, product_id1, color, size) VALUES (#{id}, #{user_id}, #{product_id}, #{adate}, #{cnt}, #{product_id1}, #{color}, #{size})")
    ...
    @Update("UPDATE cart SET user_id=#{user_id}, product_id=#{product_id}, adate=#{adate}, cnt=#{cnt}, product_id1=#{product_id1}, color=#{color}, size=#{size} WHERE id=#{id}")
    ...
    @Select("SELECT id, user_id, product_id, adate, cnt, product_id1, color, size FROM cart WHERE id=#{id}")
