...
public interface Category1Mapper {
        ...
        @Insert("INSERT INTO category1 (name) VALUES (#{name})")
        ...
        int insert(Category1Model input);

        //UPDATE문을 수행하는 메서드 정의
        @Update("UPDATE category1 SET name=#{name} WHERE id=#{id}")
        int update(Category1Model input);

        //DELETE문을 수행하는 메서드 정의
        @Delete("DELETE FROM category1 WHERE id=#{id}")
        int delete(Category1Model input);

        //SELECT문(단일행 조회)을 수행하는 메서드 정의
        @Select("SELECT id, name FROM category1 WHERE id=#{id}")
        ...
        Category1Model selectItem(Category1Model input);
        ...
        List<Category1Model> selectList(Category1Model input);

        @Select("<script>" + // <-- Dynamic SQL이 시작됨을 알림
            "SELECT COUNT(*) AS cnt FROM category1" + 
            "<where>" + // <-- 검색 조건 동적 구성 시작
            "<if test='name != null'>name LIKE concat('%', #{name}, '%')</if>" + 
            "</where>" + 
            "</script>") // <-- Dynamic SQL이 종료됨을 알림
        public int selectCount(Category1Model input);        
}
