...
public interface Category2_has_productMapper {
        ...
        @Insert("INSERT INTO category2_has_product (category2_id, product_id) VALUES (#{category2_id}, #{product_id})")
        ...
        @Update("UPDATE category2_has_product SET category2_id=#{category2_id}, product_id=#{product_id} WHERE id=#{id}")
        int update(Category2_has_productModel input);

        //DELETE문을 수행하는 메서드 정의
        @Delete("DELETE FROM category2_has_product WHERE id=#{id}")
        int delete(Category2_has_productModel input);

        //SELECT문(단일행 조회)을 수행하는 메서드 정의
        @Select("SELECT id, category2_id, product_id FROM category2_has_product WHERE id=#{id}")
        ...
        @Select("<script>" + // <-- Dynamic SQL이 시작됨을 알림
            "SELECT id, category2_id, product_id FROM category2_has_product" + 
            "<where>" + // <-- 검색 조건 동적 구성 시작
            "<if test='category2_id != null'>category2_id=${category2_id}</if>" + 
            "<if test='product_id != null'> OR product_id=${product_id}</if>" + 
            "</where>" + 
        ...
        @Select("<script>" + // <-- Dynamic SQL이 시작됨을 알림
            "SELECT COUNT(*) AS cnt FROM category2_has_product" + 
            "<where>" + // <-- 검색 조건 동적 구성 시작
            "<if test='category2_id != null'>category2_id=${category2_id}</if>" + 
            "<if test='product_id != null'> OR product_id=${product_id}</if>" + 
            "</where>" + 
