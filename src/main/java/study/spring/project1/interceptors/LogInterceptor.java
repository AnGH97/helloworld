package study.spring.project1.interceptors;
...
public class LogInterceptor implements HandlerInterceptor{
     ...
     * 클라이언트(웹 브라우저)의 요청을 컨트롤러에게 전달하기 전에 호출된다.
     * return 값으로 boolean값을 전달하는데 false인 경우
     * controller를 실행 시키지 않고 요청을 종료한다.
     * 보통 이곳에서 각종 체크 작업과 로그를 기록하는 작업을 진행한다.
     */
    @Override
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
            throws Exception {
                ...
                String clientInfo = String.format(fmt, c.device.family, c.os.family, osVersion, c.userAgent.family, uaVersion);
                ...
                //이전에 머물렀던 페이지가 존재한다면?
                //--> 직전 종료시간과 이번 접속의 시작시간과의 차이는 이전페이지에 머문 시간을 의미한다.
